...
| 04 | [툴 카탈로그](./04-tool-catalog.md) | 기본 85개 툴의 패키지별 분류와 책임 | 특정 툴 작업 전 |
...
> **작성 기준**: 이 문서는 `package.json` v0.1.0, 10개 기본 패키지 + 2개 옵션(safe-tooling) 패키지 기준입니다. 코드 식별자(함수명, 타입명, 파일 경로)는 모두 실제 소스와 1:1로 대응하며, `src/opencode/feature-package.ts`, `src/opencode/tiny-plugin.ts` 등의 실제 파일을 인용합니다.
