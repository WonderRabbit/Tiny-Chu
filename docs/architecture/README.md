...
| 04 | [툴 카탈로그](./04-tool-catalog.md) | 기본 99개 툴의 패키지별 분류와 책임 | 특정 툴 작업 전 |
...
| 10 | [GitHub 기준선 대비 요구사항 비교](./10-upstream-comparison.md) | upstream `main`과 현재 프로젝트의 기능성/비기능성/아키텍처 격차 | 구조 개선 계획 전 |
| 11 | [Feature Package 경계 재정렬 계획](./11-feature-package-boundary-plan.md) | handler 소유권과 package boundary 재정렬 | tool surface 확장 전 |
| 12 | [Runtime Mode와 Capability 계약 계획](./12-runtime-mode-capability-plan.md) | worker/orchestrator mode별 capability와 노출 계약 | runtime mode 변경 전 |
| 13 | [상태 무결성과 동시성 계획](./13-state-integrity-plan.md) | `.tiny/` writer lock, atomic write, readback 검증 | 상태 writer 변경 전 |
| 14 | [Evidence와 Artifact Governance 계획](./14-evidence-artifact-governance-plan.md) | evidence/report/artifact 검증 taxonomy | report/artifact 추가 전 |
| 15 | [설치/확장 경계 계획](./15-install-extension-boundary-plan.md) | install manifest, offline bundle, external adapter 경계 | 배포/adapter 변경 전 |
...
- 직접 런타임 의존성: `@opencode-ai/plugin`, `@opentui/solid`, `typescript` 세 개
  ...
  - `typescript`: `extractNamingSymbols()`의 compiler API runtime
...
flowchart TD
    ...
    O9["09 확장 가이드"]
    O10["10 기준선 비교"]
    O11["11 Package 경계 계획"]
    O12["12 Capability 계약 계획"]
    O13["13 상태 무결성 계획"]
    O14["14 Evidence/Artifact 계획"]
    O15["15 설치/확장 경계 계획"]
...
> **작성 기준**: 이 문서는 `package.json` v0.1.0, 14개 기본 패키지 + 2개 옵션(safe-tooling) 패키지 기준입니다. 코드 식별자(함수명, 타입명, 파일 경로)는 모두 실제 소스와 1:1로 대응하며, `src/opencode/feature-package.ts`, `src/opencode/tiny-plugin.ts` 등의 실제 파일을 인용합니다.
