...
· createTinyChuPlugin() 얇은 셸 — task_*, public_*, context_*, wiki_* 등 기본 88개 툴 노출
```
...
src/
  ...
  opencode/                     ─ 플러그인 셸 + 기본 88개 툴 (대부분의 코드)
...
> `src/opencode/`가 전체 소스의 대부분을 차지합니다. 여기에 기본 핸들러 구현 88개와 레지스트리 시스템이 모두 있습니다.
