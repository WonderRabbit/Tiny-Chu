...
· createTinyChuPlugin() 얇은 셸 — task_*, public_*, context_*, wiki_* 등 60+ 툴 노출
```
...
- **라이브 프로바이더 호출 없음** — 오케스트레이션 프로파일, agent-model 템플릿, Figma 매핑 키, Qwen 패킷 구성은 모두 **어댑터-대비만 된(adapter-ready) 메타데이터**입니다. Tiny-Chu는 네트워크 API 호출을 하지 않으며, `doctor` 준비 게이트도 로컬 전용입니다.
...
> `src/opencode/`가 전체 소스의 대부분을 차지합니다. 여기에 핸들러 구현(60+)과 레지스트리 시스템이 모두 있습니다.
