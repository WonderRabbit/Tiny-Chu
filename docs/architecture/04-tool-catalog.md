...
> 이 문서는 Tiny-Chu가 노출하는 기본 99개 툴을 패키지/카테고리별로 정리합니다. 각 툴의 핸들러는 `src/opencode/tiny-plugin.ts`의 `tools` 맵에 존재하며, 디스크립터는 `src/opencode/feature-packages/default-tool-seeds.ts`에 정의됩니다. 데이터 출처: `default-tool-seeds.ts`.
...
READ_OPTIONAL = { readOnly: true,  network: "optional" } // 명시적 provider preflight metadata probe
...
> 기본 툴은 오프라인입니다. 예외는 `provider_endpoint_preflight`의 `READ(optional network)`이며, 이 툴도 기본 `networkMode: "disabled"`에서는 요청을 보내지 않고 명시적으로 허용된 host metadata probe만 수행합니다 ([08-design-decisions.md](./08-design-decisions.md)).

---

## 패키지 1: `tiny-chu.core-runtime` (8 툴)
...
## 패키지 3: `tiny-chu.public-worker-queue` (7 툴)

위임 worker 큐 패킷과 resume packet.
...
| `public_job_resume_packet` | READ | json | checkpointed public worker job을 위한 bounded resume packet |
...
## 패키지 4: `tiny-chu.legacy-analysis` (8 툴)
...
## 패키지 5: `tiny-chu.extension-utilities` (11 툴)
...
## 패키지 6: `tiny-chu.button-workflow-hardening` (9 툴)
...
## 패키지 7: `tiny-chu.button-workflow-dispatch` (1 툴)

버튼별 worker packet을 public queue에 쓰는 dispatch surface. worker-only mode에서는 이 패키지가 제외됩니다.
...
## 패키지 8: `tiny-chu.workflow-orchestration` (12 툴)

상위 workflow source-of-truth, stop/resume, heartbeat, SOT audit.

| 툴 | 권한 | 네이티브 | 설명 |
|----|------|---------|------|
| `analysis_workflow_start` | STATE | — | repository 분석 task와 analysis workflow를 함께 시작 |
| `workflow_create` | STATE | — | durable workflow run과 첫 stop point 생성 |
| `workflow_status` | READ | — | workflow status, checkpoints, resume command 조회 |
| `workflow_checkpoint` | STATE | — | workflow node 진행을 evidence/nextSteps와 함께 저장 |
| `workflow_close` | STATE | — | evidence gate와 SOT audit 통과 후 workflow를 closed 상태로 저장 |
| `workflow_audit` | READ | — | stale run, missing evidenceRef, orphan report, checkpoint gap, projection drift를 read-only로 점검 |
| `plan_review_gate` | READ | — | workflow/worker dispatch 전 plan artifact의 objective, scope, QA, evidence, stop condition을 검증 |
| `workflow_resume_packet` | READ | — | workflow run 재개 packet 반환 |
| `workflow_packet_fit_check` | READ | — | worker packet이 context budget에 맞는지 확인 |
| `workflow_next` | READ | — | 다음 workflow packet/gate/blocked/done 상태 반환 |
| `workflow_progress_heartbeat` | READ | — | 장기 실행 workflow의 진행/대기/정체 상태 보고 |
| `workflow_sot_audit` | READ | — | final answer가 workflow source-of-truth와 evidence gate를 인용했는지 검증 |

---

## 패키지 9: `tiny-chu.small-model-resilience` (25 툴)

소형 foreman 모델의 복원력과 운영 프로파일.

| 툴 | 권한 | 네이티브 | 설명 |
|----|------|---------|------|
| `code_context_scan` | READ | — | `@TC:`/호환 `@MX:` 주석을 navigation hint로 스캔 |
| `context_budget_simulation` | READ | — | small model packet token budget 정적 추정 |
| `context_digest` | READ | rg | bounded 파일 증거 스니펫 + 인용 |
| `wiki_search` | READ | — | canonical wiki에서 citation-bearing chunk를 deterministic lexical ranking으로 검색 |
| `wiki_context` | READ | — | wiki index/query/ref 근거를 `maxChunks`/`maxChars` 예산에 맞춰 렌더 |
| `evidence_gate` | READ | — | 필수 evidence check를 pass/warning/fail로 집계 |
| `session_preflight` | READ | — | 최신 체크포인트/검증 툴/예산 원장 |
| `orchestration_profile` | READ | — | 소형 컨텍스트 OpenCode 오케스트레이션 프로파일 반환 |
| `provider_endpoint_preflight` | READ(optional network) | — | chat/generate 없이 provider metadata endpoint만 확인 |
| `qwen_retry_policy` | READ | — | qwen3.6-35b-a3b 공개 rate-limit retry/청킹 가이드 |
| `orchestration_health` | READ | — | task/worker 헬스 요약 + 진행 보존 복구 단계 |
| `dashboard_snapshot` | READ | — | task/job/workflow/provider/context/evidence 상태를 TUI용 bounded snapshot으로 요약 |
| `rules_snapshot` | STATE | — | 확인된 아키텍처 패턴을 `.tiny/rules`에 기록 |
| `small_model_contribution_evaluation` | READ | — | provider 호출 없이 offline fixture로 소형 모델 contribution rubric과 load factor를 평가 |
| `small_model_replay` | READ | — | fixture replay와 citation compliance 회귀 점수화 |
| `tool_call_conformance_probe` | READ | — | structured tool call fixture 계약 검증 |
...
| `naming_lookup` | READ | — | 이름 생성 전 canonical spelling/casing/blocked variant 조회 |
| `naming_propose` | READ | — | 후보 이름을 dictionary policy로 진단하되 상태를 쓰지 않음 |
| `naming_context` | READ | — | 이름 정책 context를 query/namespace/kind 기준으로 bounded 렌더 |
| `naming_add` | STATE | — | canonical docs를 바꾸지 않고 `.tiny/naming/events.jsonl`에 proposal event 추가 |
...
## 패키지 10: `tiny-chu.project-governance` (2 툴)

프로젝트 스냅샷과 문서-레지스트리 일관성 점검. 문서는 runtime source of truth가 아니라 composed registry의 projection으로만 취급합니다.

| 툴 | 권한 | 네이티브 | 설명 |
|----|------|---------|------|
| `project_snapshot` | STATE | — | `.tiny/project/snapshot.json`과 그 Markdown projection 작성 |
| `docs_consistency_check` | READ | — | 문서의 Tiny-Chu tool 언급을 composed registry와 비교 |

---

## 패키지 11: `tiny-chu.ux-reverse-engineering` (7 툴)
...
## 패키지 12: `tiny-chu.doctor-artifacts` (9 툴)
...
## 패키지 13: `tiny-chu.host-opencode` (0 툴)

OpenCode host adapter용 resources, instructions, hook metadata만 선언합니다. 실제 OpenCode bridge는 생성된 `registry.toolSpecs`를 소비하므로 이 패키지는 별도 tool을 소유하지 않습니다.

---

## 패키지 14: `tiny-chu.host-mcp` (0 툴)

MCP stdio host adapter용 resources와 instructions만 선언합니다. 실제 MCP descriptor와 call dispatch는 생성된 `registry.toolSpecs`를 소비하므로 이 패키지는 별도 tool을 소유하지 않습니다.
...
| core-runtime | 8 | — |
| support | 0 | — |
| public-worker-queue | 7 | — |
| legacy-analysis | 8 | — |
| extension-utilities | 11 | — |
| workflow-hardening | 9 | — |
| workflow-dispatch | 1 | — |
| workflow-orchestration | 12 | — |
| small-model-resilience | 25 | — |
...
> 정확한 툴 수는 디스크립터를 합산한 것입니다. 기본 레지스트리는 99개이며, `safeTooling`을 켜면 107개, `nativePreviews`까지 켜면 111개입니다.
