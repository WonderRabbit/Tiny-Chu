...
test("safe tooling and native previews are opt-in registry packages", async () => {
  ...
  assert.equal(Object.keys(baseline.tools).length, 70);
  assert.equal(Object.keys(nativeOnly.tools).length, 70);
  assert.equal(Object.keys(safe.tools).length, 78);
  assert.equal(Object.keys(safeNative.tools).length, 82);
