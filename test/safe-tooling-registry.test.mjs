...
test("safe tooling and native previews are opt-in registry packages", async () => {
  ...
  assert.equal(Object.keys(baseline.tools).length, 85);
  assert.equal(Object.keys(nativeOnly.tools).length, 85);
  assert.equal(Object.keys(safe.tools).length, 93);
  assert.equal(Object.keys(safeNative.tools).length, 97);
