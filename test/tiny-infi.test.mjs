...
test("context loader prefers nearest AGENTS before rules", async () => {
  ...
  await mkdir(path.join(root, ".omo", "rules"), { recursive: true });
  await writeFile(path.join(root, ".omo", "rules", "main.md"), "project rule", "utf8");
  const bundle = await loadContextBundle(root, "src/feature/file.ts");
  assert.deepEqual(bundle.documents.map((doc) => doc.path), ["src/AGENTS.md", "AGENTS.md", ".omo/rules/main.md"]);
});
...
test("wiki bundler selects canonical docs and tag matches", async () => {
  ...
  await mkdir(path.join(root, ".tiny-infi", "wiki", "domains"), { recursive: true });
  await writeFile(path.join(root, ".tiny-infi", "wiki", "domains", "backend.md"), "Backend truth", "utf8");
  const wiki = new WikiBundler(root);
  await wiki.upsertDocument({ id: "backend", path: ".tiny-infi/wiki/domains/backend.md", canonical: true, tags: ["backend"], freshness: "manual" });
...
test("tiny plugin injects context on ulw and continues unfinished plans", async () => {
  ...
  await writeFile(path.join(root, ".omo", "plans", "PLAN.md"), "# P\n\n## TODOs\n- [ ] one\n- [x] two\n", "utf8");
