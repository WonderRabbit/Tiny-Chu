...
최신값을 다시 확인할 때는 아래 명령을 실행하고, 하드코딩된 관찰값에는 항상 `observed as of YYYY-MM-DD`와 refresh command를 함께 남긴다.
...
| `tiny-chu.project-governance` | `.tiny/project` snapshot과 docs consistency check |
...
| `tiny-chu.host-mcp` | MCP stdio descriptor/call host adapter |
...
- phase 1에서 동적 package discovery, npm subpackage loading, Figma API adapter, provider chat/generate/completion call, HTTP/SSE MCP transport, MCP registry publish가 있다고 가정하지 않는다. `provider_endpoint_preflight`는 명시적으로 켜는 metadata readiness probe 예외다.
- 기본 feature package 일부를 명시적으로 제외해야 하면 `disabledPackages`를 사용한다. dependency closure를 깨거나 required package를 끄면 registry composition이 실패하고, `tiny_chu_install_check`는 `activePackages`, `excludedPackages`, `disabledPackages`, diagnostics를 함께 반환한다.
...
- 대기 중인 것처럼 보이면 모델 답을 기다리지 말고 `workflow_progress_heartbeat`를 호출한다. 완료 선언 전에는 `workflow_audit`, `evidence_gate`, `workflow_sot_audit`, `workflow_close`를 통과해야 한다.
...
| `workflow_audit` | workflow JSON SOT의 evidence gap, stale state, projection drift를 read-only로 확인할 때 | pass/fail, findings, remediationToolCalls |
| `workflow_close` | done workflow를 evidence gate 통과 뒤 closed 상태로 고정할 때 | closed/rejected, findings, sotRefs |
...
- registry parity는 direct registry, OpenCode bridge, `tiny_chu_install_check.requiredTools`가 모두 99개이고 `wiki_search`/`wiki_context`/`naming_context`/`naming_propose`는 `tiny-chu.small-model-resilience`, `wiki_bundle`은 `tiny-chu.core-runtime`인지 확인한다.
