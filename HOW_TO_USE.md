...
로컬 TUI shim은 home-screen `home_logo` plugin을 export한다.

```ts
export { default } from "../../src/opencode/tui-plugin.ts";
```

이 리포지토리 루트에서 OpenCode를 실행하면 `.opencode/plugins/tiny-chu.ts`가 로드되어 Tiny-Chu 도구가 활성화된다. `.opencode/tui.json`은 `.opencode/plugins/tiny-chu-tui.ts`를 켜서 OpenCode home-screen `home_logo`를 `TinyChu`로 바꾼다.
...
TUI `home_logo` plugin은 같은 `.opencode` layout에서 별도 TUI config와 shim으로 켠다.
...
첫 두 명령의 기대값은 `function`이다. TUI command는 `tiny-chu.logo function`을 출력해야 한다. `tiny_chu_install_check`는 OpenCode tool 노출 상태를 확인하는 운영 smoke test로 사용한다.
...
| Package | 책임 |
| --- | --- |
| `tiny-chu.core-runtime` | task, public job, context, wiki, plan 상태 |
...
| `tiny-chu.button-workflow-hardening` | button 단위 workflow, JSON-only worker result, atomic write guard |
...
- phase 1에서 동적 package discovery, npm subpackage loading, MCP server, Figma API adapter, provider API call, package disable 기능이 있다고 가정하지 않는다.
