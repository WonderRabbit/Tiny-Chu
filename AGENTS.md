...
Tiny-Chu는 OpenCode 스타일의 에이전트 작업 흐름에서 필요한 최소한의 파일 기반 오케스트레이션 기능을 제공하는 작은 TypeScript 라이브러리입니다.

이 리포지토리는 다음 기능에 집중합니다.

- 가장 가까운 `AGENTS.md`와 프로젝트 규칙 파일을 모아 컨텍스트 번들을 구성합니다.
- `.tiny/tasks/` 아래에 작업 상태를 JSON 파일로 저장합니다.
- `.tiny/plans/` 아래의 Markdown 체크박스 계획을 읽고 진행 상태를 판단합니다.
- `.tiny/public-jobs/` 아래에 public worker용 작업 큐 패킷을 저장합니다.
- `.tiny/wiki/index.json`을 기준으로 canonical wiki 문서를 선택하고 번들링합니다.
- `createTinyChuPlugin()`을 통해 task, public job, context, wiki 관련 도구를 노출합니다.
...
src/
  ...
  opencode/     Tiny-Chu 플러그인 셸 및 OpenCode 런타임 메타데이터
...
5. `test/tiny-chu.test.mjs`를 읽어 기대 동작을 테스트 관점에서 확인합니다.
...
- 상태 경로는 `resolveTinyChuPaths()`를 통해 해석합니다.
...
Tiny-Chu는 로컬 오케스트레이션 상태를 다음 위치에 기록합니다.
...
1. `test/tiny-chu.test.mjs`에 테스트를 추가하거나 기존 테스트를 갱신합니다.
2. 아래 명령을 실행합니다.
