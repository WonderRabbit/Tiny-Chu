...
export const SMALL_MODEL_TOOLS: readonly ToolSeed[] = [
  ...
  readJson("small_model_contribution_evaluation", "Evaluate offline small-model contribution rubric and load factors without provider calls."),
  ...
  readJson("naming_lookup", "Look up bounded canonical naming dictionary entries before inventing names."),
  readJson("naming_propose", "Check a candidate name against canonical dictionary policy without writing state."),
  readJson("naming_context", "Return bounded naming policy context for a query, namespace, or kind."),
